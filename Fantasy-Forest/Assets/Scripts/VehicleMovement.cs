...
public abstract class VehicleMovement : MonoBehaviour
{
    ...
    void UpdatePosition()
    {
        ...
        nextPos = position + transform.forward * radius - posToCenter;
        farNextPos = position + transform.forward * 3 * radius - posToCenter;
    }
    ...
    {
        //gets the distance
        Vector3 distance = obstacle.Pos - farNextPos;
        distance.y = 0;
        ...
        if (distance.sqrMagnitude > Mathf.Pow(obstacle.Radius + radius, 2) && distance.magnitude > (radius + obstacle.Radius))
        {
            //use a different next position instead
            distance = obstacle.Pos - nextPos;
            distance.y = 0;
    ...
    {/*
        ...
        //{
            ...
            for (int i = 0; i < 100; i++)
            {
                ...
                GL.Vertex(farNextPos);
                GL.Vertex(farNextPos + new Vector3(Mathf.Cos(angle) * radius, 0, Mathf.Sin(angle) * radius));
            }
            GL.End();
            GL.Begin(GL.LINES);
            for (int i = 0; i < 100; i++)
